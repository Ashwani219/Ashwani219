<div align="center">

# 👋 Hey, I'm Ashwani Kumar Singh

### 📊 Finance × Technology × Research

**Equity Research • Financial Markets • Python • Trading Systems • FinTech**

I build technology-driven projects around financial markets, research, and data — combining a Computer Science background with a growing focus on equity research, quantitative analysis, and systematic trading.

<a href="https://github.com/Ashwani219">
  <img src="https://img.shields.io/badge/GitHub-Ashwani219-181717?style=for-the-badge&logo=github" alt="GitHub"/>
</a>

</div>

---

## 🧠 About Me

- 🎓 B.Tech in Computer Science Engineering
- 📈 Focused on **Equity Research, Financial Analysis & Trading Research**
- 🐍 Building **Python-based backtesting and research systems**
- 💻 Interested in **FinTech, quantitative workflows and data-driven decision making**
- 📚 Continuously learning **valuation, financial modelling, derivatives and market research**

---

## 🔬 What I'm Working On

| Area | Current Focus |
|---|---|
| 📊 Equity Research | Company analysis, valuation, financial statement analysis |
| 📈 Trading Research | Strategy design, backtesting, risk management |
| 🐍 Python | Research automation, data analysis, backtesting |
| 💡 FinTech | Building practical tools at the intersection of finance & technology |

---

## ⭐ Featured Projects

### 📈 SATS Dynamic Swing Backtester
Research-oriented Python backtesting project for a trading strategy using dynamic swing anchoring and VWAP concepts.

**Focus:** strategy research • backtesting • performance analysis • quantitative workflows

### 📱 Smart Fitness
Android health-tracking application with fitness monitoring and wearable / Google Fit integration concepts.

**Focus:** Android • Java • APIs • mobile application development

### 🌐 Portfolio Website
A professional portfolio showcasing projects, skills, research work and career highlights.

**Focus:** web development • personal branding • project presentation

---

## 🛠️ Skills & Tools

**Programming**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)

**Finance & Research**

`Equity Research` `Financial Analysis` `Technical Analysis` `Derivatives` `Risk Management` `Trading Strategy Research`

**Tools**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=flat-square&logo=microsoftexcel&logoColor=white)
![Android](https://img.shields.io/badge/Android-3DDC84?style=flat-square&logo=android&logoColor=white)

---

## 📜 Certifications

- **NISM Series VIII — Equity Derivatives**
- **NISM — Currency Derivatives**
- **NISM Research Analyst — In Progress**

---

## 📌 Current Learning Roadmap

`Financial Modelling` → `Company Valuation` → `Equity Research Reports` → `Quantitative Research` → `FinTech Applications`

---

## 🤝 Let's Connect

I'm interested in opportunities and conversations around **equity research, financial analysis, trading research, quant-oriented projects, and FinTech**.

<div align="center">

### 🚀 Building at the intersection of Finance & Technology

</div>
